...
export default function ArticleCard({ post, featured = false }: ArticleCardProps) {
    return (
        <article
            className={`group card-hover ${featured ? 'md:col-span-2 md:row-span-2' : ''
                }`}
        >
            <Link href={`/blog/${post.slug}`} className="block">
                <div className={`glass-card overflow-hidden ${featured ? 'h-full' : ''}`}>
                    {/* Cover Image */}
                    {post.coverImage && (
                        <div
                            className={`relative overflow-hidden bg-muted ${featured ? 'h-96' : 'h-48'
                                }`}
                        >
                            <Image
                                src={post.coverImage}
                    ...
                    <div className={`p-6 ${featured ? 'space-y-4' : 'space-y-3'}`}>
                        ...
                        {/* Title */}
                        <h2
                            className={`font-bold text-balance group-hover:text-primary transition-colors ${featured ? 'text-3xl' : 'text-xl'
                                }`}
                        >
                            {post.title}
                        </h2>
                        ...
                        {/* Excerpt */}
                        <p
                            className={`text-muted-foreground line-clamp-${featured ? '4' : '2'
                                }`}
                        >
                            {post.excerpt}
                        </p>
